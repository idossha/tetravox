...
class Job:
    ...
    def module(self, module_id: str, op: str, **args: Any) -> "Job":
        """Run one operation of an **extension** — a first-party tool with its own panel, keys and
        files (ARCHITECTURE §13). The method keeps its historical name: `"module"` is the job-file
        action type, and renaming the wire format would break every existing job file.

        Every button in an extension's panel is also an operation, and this is the one action type
        that runs them, whichever extension and whichever operation:
        ...
        **Argument names are the manifest's, verbatim** — `radiusMm`, not `radius_mm`. The manifest
        ...
        of every extension's argument list, wrong the moment one added an argument. Where snake_case
        belongs is a typed wrapper that ships *with* the extension's vocabulary: see
        ...
        if not module_id or "." not in module_id:
            raise JobError(f"an extension id is <vendor>.<name>, not {module_id!r}")
        if not op:
