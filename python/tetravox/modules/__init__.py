...
These wrappers are where an extension's vocabulary is written down once, in Python's: snake_case
parameters, a real signature per operation, and the handful of checks worth making before an app
launch costs a second. They are **data only** — nothing here imports or inspects the extension,
which is downloaded and consented to at runtime through File ▸ Extensions… — so a wrapper is as
installable on a machine with no Tetravox as the rest of this client is.
