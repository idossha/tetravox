...
## Maintainer requirements R1–R5 (2026-08-27), mapped to owners

Source: `docs/requirements/2026-08-27-maintainer.md`. These are **Phase-2 gate items**; each owner below
writes the named E2E and the integrator runs all of them on the real data.

| Req | Owner(s) | What each owner delivers |
|---|---|---|
| **R1** mouse crosshair; 3D orbit/pan/dolly/double-click pick | **E-SCENE** | The P2-01 pointer layer in the engine (`pointerdown/move/up`, capture, per-pane hit-testing): 2D left-click/drag → `setCursor` live; 3D orbit/pan/dolly/double-click → `pick` → cursor. E2E `test/e2e/pointer.spec.ts` (synthetic pointer events; cursor ±½ voxel; other panes' `SLICE` readouts change; 3D crosshair moves). |
| **R2** pane zoom | **E-SCENE** | `⌘/Ctrl+wheel` + pinch (`wheel` with `ctrlKey`) zoom **about the pointer**, `+`/`-` about the centre, `r` fit; clamp [0.05, 20] mm/px; `SliceView.camera` persisted by `scene/serialize.ts`; corner info gains a `×zoom` readout. E2E: world point under the pointer unchanged (±0.1 mm) after a zoom step. |
| **R3** crosshair, not the scan | **E-SCENE** | Left-drag never changes `camera.center`; pan = middle-drag / `space`+drag / two-finger drag; right-drag = window/level on the active layer (`Shift`+drag = opacity). E2E: byte-identical pixel away from the crosshair after a left-drag; `camera.center` unchanged; middle-drag moves centre, not cursor. |
| **R4** mesh cross-sections in 2D panes, sweepable | **E-MESH** (cut-manager serves *named plane requests per consumer*: `requestCut(datasetId, key, planes, opts)` with latest-wins per key, so the three panes and the 3D clip planes never starve each other) · **E-DERIVED** (`fillIn2D` per-element polygons with tag/field colour honouring `tagStyle` + isolation mask; `contoursIn2D` tissue-boundary lines; instanced line renderer; **defaults on when a mesh is opened**) · **E-SCENE** (slice stepping works with no volume loaded — step 1 mm, scene bounds come from meshes; `stepCursor` and wheel-slice do not require a volume) · **A-PROPS** (fill/contour toggles, field selector for the cut colour). Gate: `ernie.msh` alone → tissue cross-sections in all three panes with `.msh.opt` colours (analytic pixel at known RAS points); `Thalamus_TI.msh` with `TI_max` on the cut cross-checked through `locate`; 20-step axial sweep ≥ 30 fps end-to-end (measured, `docs/benchmarks/phase2.md`); goldens mesh-only and T1 + mesh. |
| **R5** region select / mute / recolour | **A-PROPS** (the Region panel: rows with eye · colour picker · opacity · name · id · count; search; click select, ⇧/⌘ multi, Alt solo, Show all / Hide all / Invert, double-click → centroid via `labelCentroids`; one panel for label volumes, mesh tags and annots) · **E-SLICE** (`visibleLabels`, `labelOpacity`, recolour = patched `labelLut` → palette re-upload, selected-label outline emphasis) · **E-MESH** (`tagStyle` visible/opacity/colour per tag; `colorMode:'label'` table edits; selected-tag edge emphasis) · **E-SCENE** (pick/probe results carry the label id / tag so a click in a pane selects the row; selection + colour edits round-trip through scene save/load) · **A-SHELL** (Save LUT… export of edited colours). Gate: hide → those pixels change, others byte-identical; recolour → exactly those pixels become the new colour; solo; the same on a mesh tag in 3D and in a 2D cut; persists through save/load. |

No frozen-type change is needed: recolouring edits the layer's own `LabelTable` (`VolumeLayer.labelLut`, `MeshLayer.label.table`) and `tagStyle.color`; visibility uses `visibleLabels` / `tagStyle.visible`.
