...
| `packages/app/e2e/module-job.spec.ts` | A `--job` launch that activates an extension and runs its operation, asserted through `job-result.json` and the scene it saved. Runs everywhere: the data is `testdata/`. |
