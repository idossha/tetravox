...
| 2026-09-18 ROI overlays | §4.2/§6.2/§7.4; `mesh-threshold.test.ts` + `mesh-threshold-open.spec.ts` (+ golden) pin the one-open-bound gate; `defaults.test.ts` pins SimNIBS one-view seeding by `toEqual` beside the `Visible = 1` view seeding for mesh and surface layers; `cut.rs::valued_tests`, `store.test.ts` and `surface-contours-scalar.spec.ts` (grey per segment midpoint from the fixture's own field rule, threshold, golden) cover the scalar outline; `surface-contours-scalar-real.spec.ts` opens a real `roi_overlay.msh` bare and asserts the outline is inside the ROI box only, in many colours, never yellow. |
