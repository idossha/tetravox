...
'tetravox-module-<id>')` from the renderer stays in *this* renderer process and this JS realm, so the panel is
rendered by the same React root, out of the same `ModuleInstance`, over the same `ModuleHost`. That is what
keeps §12.3 item 6's freeze intact: the host stays **synchronous**, and pop-out is a layout change rather than
the execution-model change §13.9 prices. A second `BrowserWindow` with its own renderer was the alternative
and is rejected on correctness, not on cost — it needs a second `activate()` over one scene, and two contact
editors over one electrodes table is a merge conflict, not a feature.

Four consequences the implementation has to honour, each of which is a bug if it is dropped:
